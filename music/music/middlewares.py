...
class RandomUserAgentMiddleware(object):  #改写随机useragent
...
class MyAutoProxyDownloaderMiddleware(object):
    ...
    def get_random_proxy(self):
        proxy = "https://" + random.choice(self.proxy_list)
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status in self.proxy_status:
            ...
            if not request.meta.get('auto_proxy'):
                request.meta.update({'auto_proxy': True, 'proxy': self.get_random_proxy})  #self.proxy_config获取得应该是https://127.0.0.1:8877得方式
